(ns box.main
  "Kotodama WASM entrypoint for the Box clean-room actor (L4) — Clojure port.
  ...
  facts (`box.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "File"          :plural "files"          :id-prefix "box_fil"
    :fields [:name :description :size :sha1 :itemStatus :commentCount]
    :required [:name :description]
    :coerce {:size :int :commentCount :int}  :refs {}}
   {:entity "Folder"        :plural "folders"        :id-prefix "box_fol"
    :fields [:name :description :size :itemStatus :syncState]
    :required [:name :description]
    :coerce {:size :int}                      :refs {}}
   {:entity "User"          :plural "users"          :id-prefix "box_use"
    :fields [:name :login :status :role]
    :required [:name :login]
    :coerce {}                                :refs {}}
   {:entity "Collaboration" :plural "collaborations" :id-prefix "box_col"
    :fields [:role :status :isAccessOnly]
    :required [:role :status]
    :coerce {:isAccessOnly :bool}             :refs {}}
   {:entity "SharedLink"    :plural "sharedlinks"    :id-prefix "box_sha"
    :fields [:access :vanityName :canDownload]
    :required [:access :vanityName]
    :coerce {:canDownload :bool}              :refs {}}
   {:entity "Comment"       :plural "comments"       :id-prefix "box_com"
    :fields [:message :isReplyComment]
    :required [:message :isReplyComment]
    :coerce {:isReplyComment :bool}           :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"box.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "box-compat" :tier tier :entities entities} 200])
